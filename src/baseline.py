...
def fillUtilityMatrix(utility_matrix, similarity_matrix, top_k):
    ...
    for i in tqdm(range(0, len(original_matrix))):
        ...
        for j in range(len(row)):
            ...
            if math.isnan(rating):  # Update only empty entries in utility matrix
                indices, weights = top_K_highest_values(similarity_matrix[i], k=top_k + 1)
                # Since the maximum similarity of a user is with himself,
                # we get k+1 top values and discard the maximum
                indices = indices[1:]
                weights = weights[1:]
...
if __name__ == '__main__':
    ...
    matrix = f'../data/utility_matrices/people_utility_matrix.csv'
